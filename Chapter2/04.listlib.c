...
int accessdata(void){ // return positive if success
    int i;
    list_t **newptrs;
    if (headptr==NULL){ // cannot access at completely null list
        errno = EINVAL;
        return -1;
    }
    if(travptrs_size == 0){ // first traverse
    ...
    for(i=0; i<travptrs_size;i++){// set null space to headptr
        if(travptrs[i] == NULL){
            travptrs[i] = headptr;
            return i;
        }
    ...
    if(newptrs == NULL) return -1; // if cannot unfold trav key array
    travptrs = newptrs;
...
int adddata(data_t data){
    ...
    if((newnode=(list_t*)(malloc(nodesize)))==NULL) return -1;
